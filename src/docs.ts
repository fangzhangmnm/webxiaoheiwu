...
export async function rekeyDoc(name: string, newPassword: string): Promise<{ status: string }> { const r = await file(name).rekey({ newPassword }); invalidateEncryptedFlag(name); return r; }
