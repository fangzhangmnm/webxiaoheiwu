...
- **v2 主干树 + links（2026-09-10 深夜，ADR-0014；v2.1.1 落地，SSoT = `ai-docs/20260910-v2-tree-schema-handoff.md` + `20260910-v2-ui-handoff.md` §4 末）**：graph.json `version: 2` = 顶层 `tree`（嵌套数组，一页至多一个父亲、兄弟有序、成员可选 = 散页合法；无 folder / 节点类型 / compile 标记）+ `pages[名].links`（指向）。一句话：**兄弟 = 顺序，孩子 = 归属，link = 指向**。`version !== 2` 一律拒开、**零 legacy 分支**；占位符废止（打新名 = 当场建空文件）；删除 = 三动词（见 2.0 bullet），移出树 = 子树边降级成 links。侧栏 = 当前页邻域（`..` / 兄弟 / 子节 / 链接 / 谁指向这里），**不铺整棵树**；树移动六件走行菜单（拖拽等手感再议）；上一页 / 下一页 = 全树前序 DFS（**章节名两侧 ‹ › chevron**，v2.1.3 从页脚搬上来；首尾不绕回，散页灰）；「导出这一支」= 子树 DFS 拼 txt；「+」拆成 + 兄弟 / + 子节。不做：全树视图 / 孤儿面板 / 计数 / 自动归档 / 自动起名 / shelf / edges / dim。夹具 = `tmp/migration/` 四本（`verify.mjs` 回归；不进 git）。edited by Claude Fable 5.1 2026-09-10
