...
- **store**：`@internal/store` 0.12.0 + `@internal/encryption` 0.1.0（`vendor-pkgs/` tgz）。唯一接缝 `src/app-store.ts`（值级 import 只准这里；`src/encryption.ts` 是加密器官接缝）；`scripts/build.sh` 与 `test/redline-guard.test.mjs` 守着。缺接口 escalate 改库，绝不在 app 端绕。升级 = `bash "../20260813 internal-store/scripts/pull-package.sh" <ver>`。
