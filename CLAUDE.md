...
- **2.0 书模式（2026-09-10，user 真机三轮打回后的形状；SSoT = `ai-docs/20260910-v2-ui-handoff.md` §4 末 + ADR-0008–0011 及其 09-10 修订）**：用户面名词 = **书**（zip）/ **页**（节点；graph.json 键 09-10 已吃书成 `pages`）。**顶栏最左 = 书库钮**（v2.1.7，user「标题左边放书库图标。标题还是改名」；名字钮仍 = 改名）；☰ 在顶栏最右 = 侧栏（默认关、纯浮层不推纸面、右侧；顶 = 书库/设置；工程内导航 = 当前页出边列表，行小字 = 修改时间，末尾「+」加页；回退/进页不自动收）；☰ 左边「+」= 这一页菜单（加兄弟页 / 加子节 / 挪到…）；页名 = 纸面顶部章节名框（改了即改名）；新页/分裂都弹名字框（placeholder 提示下一章号，按语言生成，不预填）；新边加末尾；显示无 .txt；**删除模型 = 断开链接 / 废弃（改名 `_废-`，树里连同子树）/ 彻底删除（只对 `_废-`，清入链）三个显式动词，无引用计数无孤儿概念**（09-10 深夜改写，ADR-0014 §8；书里无 .trash）；editor-state `{last, back}` 不标脏保存随手捞；**修改锁跟着作品**（graph.json `readOnly`，txt 无锁）；容器 = `pages/` + graph.json `pages`（09-10 吃书，旧 `contents/` 拒开）；加页名字框不提示章号，只有第一页有默认名。**z-index 只准出现在 `styles.css` 顶部 `:root` 的 `--z-*` band 表**（test 执法，抄 WeebPaint）。改 UI 后跑 `node tools/ui-audit.mjs` 再交。edited by Claude Fable 5.1 2026-09-10
- **2.1 图片页 + 封面（2026-09-10，user 两圈 grill；ADR-0012 / ADR-0013）**：`pages/*.jpg|png|webp|gif` 与 txt 同表，GIF 动图直通；进门单一漏斗 `src/image/`（魔数嗅探；长边 ≤2048 只剥 metadata、>2048 缩 + JPEG q85、「保留高清」勾 4096；EXIF 方向≠1 重编码；GIF >2 MB 二次确认；撞名 `名-hex4.ext`，txt 稿 / 书撞名同改 hex4）；解码边界 = `src/image/codec.ts`（app 唯一 canvas 点，重采样走 gallery 包、编码走 vendored jpeg-js / UPNG）；**封面 = `Thumbnails/thumbnail.png` 本身**（最后一个 entry、≤256²、≤70 KB，iTXt Description = 腰封；graph.json 无 cover 字段；「设为封面」/「替换图片」在图片页下方；加密书封面走 store `makePeek` 密文尾片）；书库缩略图 = gallery `policy.thumbs`（IDB `webxiaoheiwu-thumbs`）、tile 2:3 窄屏三列、角标「打开中」；侧栏「谁指向这里」入边段可「断开」（纯 unlink）；加页 sheet 副按钮「从图片…」（多选）；拖放 / 粘贴同漏斗；页脚字数统计（设置可关）；书库出 → 回来在书库（device-kv `last-scene`）；**书库顶栏露出云状态钮 + 刷新钮**（与抽屉头同一颗云菜单；user 2026-09-10「外面和菜单里都有」= WeebPaint 图库 header 同款）；侧栏页头 回退 + **前进**（浏览器语义、内存态）；话筒悬浮、字数统计露着时上让 38px（Quest 竖向空间紧）。edited by Claude Fable 5.1 2026-09-10
- **v2 主干树 + links（2026-09-10 深夜，ADR-0014；v2.1.1 落地，SSoT = `ai-docs/20260910-v2-tree-schema-handoff.md` + `20260910-v2-ui-handoff.md` §4 末）**：graph.json `version: 2` = 顶层 `tree`（嵌套数组，一页至多一个父亲、兄弟有序、成员可选 = 散页合法；无 folder / 节点类型 / compile 标记）+ `pages[名].links`（指向）。一句话：**兄弟 = 顺序，孩子 = 归属，link = 指向**。`version !== 2` 一律拒开、**零 legacy 分支**；占位符废止（打新名 = 当场建空文件）；删除 = 三动词（见 2.0 bullet），移出树 = 子树边降级成 links。侧栏 = 当前页邻域（`..` / 兄弟 / 子节 / 链接 / 谁指向这里），**不铺整棵树**；树移动六件走行菜单（拖拽等手感再议）；上一页 / 下一页 = 全树前序 DFS（**章节名两侧 ‹ › chevron**，v2.1.3 从页脚搬上来；首尾不绕回，散页灰）；「导出这一支」= 子树 DFS 拼 txt；「+」拆成 + 兄弟 / + 子节；任何页 = 「挪到…」（v2.1.7：通用 pick sheet `sheets.ts openPickSheet` 搜主干锚点 → 放到它之下 / 之后 / 书的末尾；书的末尾 = 空树唯一入口，v2.1.6「归入主干」并入；三处入口 = 散页侧栏首行 / 顶栏「+」菜单 / 树行 ⋯；命中行只显示名字，user「key 是唯一的」）。不做：全树视图 / 孤儿面板 / 计数 / 自动归档 / 自动起名 / shelf / edges / dim。夹具 = `tmp/migration/` 四本（`verify.mjs` 回归；不进 git）。edited by Claude Fable 5.1 2026-09-10
- **书的落盘 / 推云节律（ADR-0015，v2.1.2；出处 = 图片 session 报告转述的 user 拍板）**：IDB 与云不同心跳（推云 15 s / 30 s 不动）；增量重打 = `rawCache` + zip.js passThrough（未改 entry 原样塞回，仍整包重写、同内容同字节）；切页即本地落盘（不额外推云）；书的本地防抖随上次落盘耗时放缓（200 ms 起封顶 3 s，txt 常量不动）；async 上传合并 = store 库改动待 user 立项。edited by Claude Fable 5.1 2026-09-10
