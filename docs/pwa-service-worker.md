...
Skip cross-origin requests entirely in the fetch handler. MSAL CDN, Microsoft Graph, etc. must pass through to the browser. Filter on `url.origin !== self.location.origin` and `return` (no `event.respondWith`).
...
- Don't precache CDN assets (MSAL). Their cache headers and your SW lifecycle don't compose well; let the browser HTTP cache handle them.
