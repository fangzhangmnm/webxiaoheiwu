...
> created 20260903 · by Claude Fable 5.1 · status: accepted · **标题可见性 2026-09-04 已收口 → ADR-0007**（加密稿名字改日期码，标题不进名字；容器/密码模型本 ADR 不变）
...
## 取舍（明说）
- ~~**标题在文件名里可见**（v1 藏进密文）。理由：对齐 WeebPaint（`X.ora.zip`）、零 app 侧容器知识、列表不必逐份解密。~~ **2026-09-04 superseded by ADR-0007**：文件名是管理句柄不是标题，加密稿出生名 = `yyyymmdd-hex4`，转加密自动改日期码。列表显真标题的 peek 增强（需 store `RawFile.readPeek`）未做。
