...
> ⚠ as-of v0.2.3 / 2026-09-03（edited by Claude Fable 5.1）：实现 = `src/ime.ts` + `src/app.ts`；用户词库同步走 `rime-user-dict` collection。**2026-09-03 修订（user「直接不用系统输入法了…隐私 paranoid」）**：下面「平台矩阵 / opt-in」一节已过时——内置 IME **全平台默认开**，编辑区全平台 `inputmode=none`，设置页「改用系统输入法」是 per-device 逃生开关（默认关；裸 iPad 无实体键盘时用，软键盘以后做）；系统输入法插手（`compositionend`）→ 撤掉那段 + 状态栏提示。方案：全拼（默认）/ 全拼·语句流 / 微软·自然码·小鹤·ABC·拼音加加 双拼 / 五笔 86（user「就全拼微软」「加一个五笔玩玩」「双拼顺手支持其他方案」；不开笔画，但 `stroke` 文件必须留：worker 清单里 luna_pinyin 反查硬依赖它）。
...
- We want zero CDN dependencies for the entire app shell, including sign-in (MSAL is also vendored — see [20260524-msal-onedrive.md](20260524-msal-onedrive.md)).
...
[20260524-push-to-talk.md](20260524-push-to-talk.md#ime-composition-leak-war-the-long-detour)
