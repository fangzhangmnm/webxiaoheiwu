...
try {
  ...
  const kbBack = await page.evaluate(async () => { const w = (ms) => new Promise((r) => setTimeout(r, ms)); window.dispatchEvent(new Event("blur")); await w(50); const noHint = !document.body.classList.contains("kb-away") && !document.querySelector(".kb-away-hint"); document.activeElement?.blur(); window.dispatchEvent(new Event("focus")); await w(50); return { noHint, c: document.activeElement === document.getElementById("editor") }; });
