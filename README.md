...
- **加密稿**：一篇一篇选，加密后网盘和本机存的都是密文（7-Zip 格式，用 7-Zip 输密码也能打开）。密码只在内存里，忘了就真的找不回。
- **语音输入（全本机）**：识别在你的设备上跑，声音不出设备，加密稿也能用。设置里一次下载语音包（SenseVoice 229 MB，或 30 MB 的实验小模型），之后离线可用；模型源可换镜像，字节到手都先校验。按住左 Ctrl 说话。识别模型 SenseVoiceSmall（阿里巴巴通义实验室，FunASR 模型开源协议）；运行时 sherpa-onnx（Apache-2.0）。
