// probe/asr/sha256.js —— 流式 SHA-256（纯 JS，字节进十六进制出；update() 可分片喂，O(1) 内存）。
// 用途：模型分片到手先验再落盘/入 FS——对手是「被黑的镜像站」，所以必须是密码学哈希；不用 crypto.subtle.digest
// 是因为它非流式（要整块在内存）且把「切片大小」绑在了内存上。created 2026-09-03 by Claude Fable 5.1
...
export class Sha256 {
  constructor() {
    ...
    this.buf = new Uint8Array(64); this.bufLen = 0; this.total = 0; this.w = new Uint32Array(64); this.done = false;
  }
